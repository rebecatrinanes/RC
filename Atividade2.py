...
img2 = cv2.imread('madfoxera.jpg',0)  # Imagem do cenario - puxe do video para fazer isto
...
#image = cv2.imread("madfoxera.jpg")
#gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)

# detect circles in the image
circles = cv2.HoughCircles(img2, cv2.HOUGH_GRADIENT, 1.2, 100)
 
#sucesso = cv2.imread('sucesso.jpg')  
...
if circles is not None:
    ...
    if len(good)>MIN_MATCH_COUNT:
        ...
        print('Sucesso! A folha foi detectada')
    else:
        print('Ops! A folha não foi detectada')
else: 
    print('Ops! A folha não foi detectada')
    ...
    #cv2.imshow("output", np.hstack([img2]))
    #cv2.waitKey(0)
